...
summary: 在 Cisco Nexus 9364D-GX2A 交換器上安裝 NX-OS，用於新的 AFX 1K 或 AFX 2K 系統安裝、交換器更換或恢復原廠預設值。 
---
= 在 Cisco Nexus 9364D-GX2A 交換器上安裝 NX-OS
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 9364D-GX2A 交換器上安裝 NX-OS，用於新的 AFX 1K 或 AFX 2K 系統安裝、交換器更換或將交換器重設為原廠預設值。
...
+
** 如果您的交換器已安裝目前 NetApp 支援的版本，則無需安裝 NX-OS。您需要安裝 RCF。首先執行link:configure-install-upgrade-rcf-overview-9364d.html["安裝或升級 RCF 概述"]。
...
.下一步是什麼？
在交換器上安裝 NX-OS 後，您需要從 link:configure-install-upgrade-rcf-overview-9364d.html["安裝或升級 RCF 概述"] 開始安裝 RCF。
