...
summary: 您可以使用此程序在 Nexus 9364d-gx2a 叢集交換器上安裝或升級 NX-OS 軟體。 
---
= 安裝或升級 NX-OS 軟體
:allow-uri-read: 
:icons: font
...
[role="lead"]
請依照下列步驟在 Nexus 9364D-GX2A 交換器上安裝或升級 NX-OS 軟體。
...
* 完成該程式link:install-nxos-overview-9364d-cluster.html["準備安裝 NX-OS 和 RCF"]。
* 交換器配置的目前備份可用。
* 一個功能齊全的叢集（日誌中沒有錯誤或類似問題）已經到位。
...
. 將 NX-OS 軟體和 EPLD 映像複製到 Nexus 9364D-GX2A 交換器。
+
.顯示範例
[%collapsible]
====
...
.下一步是什麼？
安裝或升級 NX-OS 軟體後，您可以link:install-upgrade-rcf-overview-cluster-9364d.html["安裝或升級參考設定檔（RCF）"]。
