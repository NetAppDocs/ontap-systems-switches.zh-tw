...
summary: 如果您已有雙節點無交換器叢集環境，則可以使用Cisco Nexus 9364d-gx2a 交換器遷移到雙節點有交換器叢集環境，以便能夠將叢集擴展到兩個以上節點。 
---
= 遷移到雙節點交換集群
:allow-uri-read: 
:icons: font
...
[role="lead"]
如果您已有雙節點無交換器叢集環境，則可以使用Cisco Nexus 9364D-GX2A 交換器遷移到雙節點有交換器叢集環境。
...
** 請參閱 link:https://hwu.netapp.com["Hardware Universe"^] 以取得所有支援的 ONTAP 版本。
...
+
如需佈線的詳細資訊、請參閱 link:https://hwu.netapp.com["Hardware Universe"^]。
...
請參閱 link:https://hwu.netapp.com["Hardware Universe"^] 以取得有關您平台叢集連接埠的資訊。
