...
summary: 完成初始組態並套用任何必要的授權後，請在 Broadcom BES-53248 叢集交換器上安裝參考組態檔案 （RCF）。 
---
= 在 Broadcom BES-53248 叢集交換器上安裝 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成初始組態並套用所有必要的授權後，請在 Broadcom BES-53248 叢集交換器上安裝參考組態檔案（RCF）。RCF 定義叢集運作所需的連接埠分配、VLAN 和網路設定。


CAUTION: 對於 EFOS 3.12 及更新版本，請遵循 link:efos-install-rcf-license-file.html["安裝參考組態檔（RCF）和授權檔案"] 中的安裝步驟。
...
* 目前的 RCF 檔案可從 link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["博通集群交換機"^] 頁面取得。
* RCF 中的啟動配置反映了所需的啟動映像，如果您僅安裝 EFOS 並保留目前的RCF 版本，則需要此配置。如果需要更改啟動配置以反映目前的啟動映像，則必須在重新套用 RCF 之前進行更改，以便在以後的重新啟動中實例化正確的版本。
* 從出廠預設狀態安裝 RCF 時，需要透過控制台連接到交換器。如果您已事先使用知識庫文章link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["如何在保持遠端連線的情況下清除博通互連交換器上的配置"^]清除配置，則此步驟為可選。


.建議的文檔
請查閱交換器相容性列表，以了解支援的 ONTAP 和 RCF 版本。請參閱link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["EFOS軟體下載"^]。請注意，RCF 中的命令語法與 EFOS 版本中的命令語法可能存在依賴關係。
...
* 本流程範例中使用了四個節點。這些節點使用兩個 10GbE 叢集互連連接埠 `e0a`和 `e0b`。請參閱link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]以確認您平台上的正確叢集連接埠。
...
CAUTION: 在安裝新的交換器軟體版本和 RCF 之前，請使用知識庫文章 link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["如何在保持遠端連線的情況下清除博通互連交換器上的配置"^]。如果您必須完全清除交換器設定，則需要再次執行基本組態。您必須使用序列主控台連線至交換器，因為完全清除組態會重設管理網路的組態。
...
. 請注意以下事項：
+
.. 如果交換器上已安裝額外的連接埠授權，則必須修改 RCF 以設定這些額外的授權連接埠。如需詳細資訊，請參閱 link:configure-licenses.html#activate-newly-licensed-ports["啟動新授權的端口"]。
...
--
====
. [[step20]]在叢集交換器 cs1 上，關閉與節點叢集連接埠連接的連接埠。
+
以下範例使用介面範例輸出：
...
.下一步是什麼？
安裝 RCF 後，您可以… link:configure-ssh.html["啟用 SSH"]。
