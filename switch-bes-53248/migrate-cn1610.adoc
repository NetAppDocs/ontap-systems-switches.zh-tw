...
summary: 依照移轉需求和步驟，將現有的 NetApp CN1610 叢集交換器移轉至 Broadcom BES-53248 叢集交換器。 
---
= 將 NetApp CN1610 叢集交換器移轉至 Broadcom BES-53248 叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將現有的 NetApp CN1610 叢集交換器移轉至 Broadcom BES-53248 叢集交換器。請先查看移轉要求，然後依照移轉步驟進行交換器更換，確保叢集運作不會中斷。
...
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 包含有關 ONTAP 相容性、支援的 EFOS 韌體以及與 BES-53248 叢集交換器連接的資訊。如需交換器安裝需求的詳細資訊,請參閱 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^]。
