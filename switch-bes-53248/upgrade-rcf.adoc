...
. 請注意以下事項：
+
.. 如果交換器上安裝了額外的連接埠許可證，則必須修改 RCF 以設定額外的許可證連接埠。看 link:configure-licenses.html#activate-newly-licensed-ports["啟動新授權的端口"] 詳情請見下文。但是，升级到 RCF 1.12 或更高版本时，不再需要进行这些修改，因为所有接口现在都已预先配置。
