...
summary: 升級 EFOS 軟體後，升級 Broadcom BES-53248 叢集交換器上的參考組態檔案 （RCF）。 
---
= 升級 Broadcom BES-53248 叢集交換器上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在升級 EFOS 軟體並套用任何必要的授權後，升級 Broadcom BES-53248 叢集交換器上的參考組態檔案（RCF）。
...
* 有關 BES-53248 交換器升級和降級程序的完整文檔，請參閱 link:https://www.broadcom.com/support/bes-switch["博通"^] 網站上提供的相應軟體和升級指南。
...
您可以使用交換器 CLI 和 ONTAP CLI （如下列步驟所示）進行連接埠組態設定。首先在第一台交換器 （cs2） 上設定連接埠，然後對第二台交換器 （cs1） 重複這些步驟。
...
CAUTION: 更新或套用新的 RCF 時，必須清除交換器設定並執行基本組態。必須使用串列主控台連接到交換器才能清除交換器設定。
...
. 請注意以下事項：
+
.. 如果交換器上安裝了額外的連接埠授權、則必須修改 RCF 以設定額外的授權連接埠。如需詳細資訊、請參閱 link:configure-licenses.html#activate-newly-licensed-ports["啟動新授權的端口"]。不過、當您升級至 RCF 1.12 或更新版本時、就不再需要修改、因為所有介面現在都已預先設定。
...
--
. 使用下列傳輸協定之一將 RCF 複製到交換器 cs2：HTTP、HTTPS、FTP、SFTP 或 SCP。
+
...
.下一步是什麼？
升級 RCF 後，您可以link:configure-ssh.html["啟用 SSH 以收集日誌"]。
