...
summary: Broadcom BES-53248 叢集交換器基本型號支援 16 個 10GbE 或 25GbE 連接埠和 2 個 100GbE 連接埠。安裝額外的連接埠授權以擴充交換器容量。 
---
= 安裝 Broadcom BES-53248 叢集交換器的授權
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248 叢集交換器基本型號已獲授權支援 16 個 10GbE 或 25GbE 連接埠及兩個 100GbE 連接埠。安裝額外的連接埠授權即可擴充交換器，以支援更多 10GbE 或 25GbE 叢集連接埠。
...
NOTE: 若要使用交易金鑰兌換連接埠授權金鑰檔案，請參閱 link:https://support.broadcom.com/web/csg/license-generation["博通支援的乙太網路交換器的授權門戶"^]。如需更多詳細資訊，請參閱知識庫文章 link:https://kb.netapp.com/on-prem/Switches/Broadcom-KBs/How_to_add_Additional_Port_Licensing_for_the_Broadcom_BES_53248_Switch["如何為博通 BES-53248 交換器添加額外的連接埠許可"^]。
