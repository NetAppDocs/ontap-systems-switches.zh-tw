...
summary: 將雙節點無交換器式叢集移轉至使用 Broadcom BES-53248 叢集交換器的交換式叢集環境，以實現擴充至兩個節點以上的規模。 
---
= 使用 Broadcom BES-53248 叢集交換器移轉至雙節點交換式叢集
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用 Broadcom BES-53248 叢集交換器，將現有的雙節點無交換器式叢集移轉至有交換器叢集環境。這種不中斷營運的移轉可讓您將叢集擴充至兩個節點以上。
...
+
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 包含有關 ONTAP 相容性、支援的 EFOS 韌體以及與 BES-53248 交換器連接佈線的資訊。如需交換器安裝要求的詳細資訊、請參閱 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^]。
...
+
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 包含有關您平台實際叢集連接埠的最新資訊。
...
+
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 包含有關佈線的更多資訊。
