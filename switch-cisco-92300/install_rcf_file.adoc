...
[NOTE]
====
* 此程序需要同時使用ONTAP 支援指令和 https://["Cisco Nexus 9000系列交換器"^]；ONTAP 除非另有說明、否則會使用指令。
...
CAUTION: 更新或套用新的RCF時、您必須清除交換器設定並執行基本組態。如果您可以停用 PowerOn Auto Provisioning （ POAP ）（需要 NX-OS 9.2P1 或更新版本）、 SSH 交換器存取將會保留。為了額外的預防措施、如果在交換器重新開機後無法存取 SSH 、請使用序列主控台連線到交換器。
...
====
+
請參閱 https://["使用 PowerOn Auto Provisioning"^] 以取得更多詳細資料。
...
. 使用下列傳輸傳輸協定之一、將RCF複製到交換器CS2的bootflash：FTP、TFTP、SFTP或scp。如需Cisco命令的詳細資訊、請參閱中的適當指南 https://["Cisco Nexus 9000系列交換器"^] 指南：
+
...
+
如需Cisco命令的詳細資訊、請參閱中的適當指南 https://["Cisco Nexus 9000系列交換器"^] 指南：
...
輸入： ``system switch ethernet log setup-password``
...
Enter the password again: <enter switch password>
----
接著： ``system switch ethernet log enable-collection``
...
.適用於更新版本的更新版本ONTAP
--
對於支援支援支援支援更新版本的版本、請啟用叢集交換器健全狀況監控記錄收集功能、以便使用下列命令來收集交換器相關的記錄檔：ONTAP
...
Enter the password again: <enter switch password>
----
接著： ``system cluster-switch log enable-collection``
