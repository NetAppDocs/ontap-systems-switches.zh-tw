...
summary: 更換叢集網路中發生故障的 Cisco Nexus 92300YC 叢集交換器。此為不中斷營運的程序。 
---
= 更換 Cisco Nexus 92300YC 叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中斷叢集運作的情況下，更換叢集網路中發生故障的 Cisco Nexus 92300YC 叢集交換器。此不中斷營運的操作 （NDU） 包括準備一台替換交換器並將連線從故障交換器移轉過來。
...
* 適用於 Nexus 92300YC 的替換交換器：
+
** 替換交換器的管理網路連線功能正常。
** 控制台已就緒，可存取替換開關。
** 節點連接埠為 1/1 至 1/64。
** 連接埠 1/65 和 1/66 上的所有交換器間連結 (ISL) 連接埠均已停用。
** 所需的參考組態檔案（RCF）和 NX-OS 作業系統映像交換器已載入到交換器上。
** 交換器的初始自訂已完成，詳情如下：link:configure-install-initial.html["設定 Cisco Nexus 92300YC 交換機"]。
+
...
* 現有的 Nexus 92300YC 交換器的名稱是 cs1 和 cs2。
* 新款 Nexus 92300YC 交換器的名稱是 newcs2。
...
.. 按一下瀏覽器的返回箭頭，返回 *Description* 頁面，按一下 *CONTINUE*，接受授權合約，然後前往 *Download* 頁面。
...
----
====
. 從 Nexus 92300YC cs2 交換器上拔除所有纜線，然後將它們連接到 Nexus 92300YC newcs2 交換器上的相同連接埠。
. 啟動 cs1 和 newcs2 交換器之間的 ISL 連接埠 1/65 和 1/66，然後驗證連接埠通路運作狀態。
+
連接埠通道應指示 Po1(SU)，成員連接埠應指示 Eth1/65(P) 和 Eth1/66(P)。

+
.顯示範例
[%collapsible]
====
此範例啟用 ISL 連接埠 1/65 和 1/66，並在交換器 cs1 上顯示連接埠通路摘要：
