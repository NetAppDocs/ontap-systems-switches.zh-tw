...
summary: 在 Cisco Nexus 92300YC 叢集交換器上，初始設定後或有新版本可用時，安裝或升級參考組態檔案 （RCF）。 
---
= 在 Cisco Nexus 92300YC 叢集交換器上安裝或升級 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 92300YC 叢集交換器上完成初始設定後，安裝參考組態檔案 （RCF），或在有新版本可用時升級您的 RCF 版本。
...
* 節點名稱是 `node1`和 `node2`。
* 叢集 LIF 名稱為 `node1_clus1`、 `node1_clus2`、 `node2_clus1`及 `node2_clus2`。
* 這 `cluster1::*>`prompt 指示叢集名稱。


[NOTE]
====
* 此程序需要同時使用 ONTAP 命令和 https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["CiscoNexus 9000 系列交換機"^]；除非另有說明，否則使用 ONTAP 命令。
* 在執行此程序之前，請確保您已備份目前的交換器組態。
...
CAUTION: 更新或套用新的 RCF 時，必須清除交換器設定並執行基本組態。您必須連接到交換器的序列控制台連接埠才能重新設定交換器。
...
. 使用下列傳輸協定之一將 RCF 複製到交換器 cs2 的 bootflash：FTP、TFTP、SFTP 或 SCP。如需 Cisco 命令的詳細資訊，請參閱 link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["CiscoNexus 9000 系列交換機"^] 中的適當指南。
+
...
+
如需 Cisco 命令的詳細資訊，請參閱 link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["CiscoNexus 9000 系列交換機"^] 中的適當指南。

+
此範例顯示在交換器 cs2 上安裝 RCF 檔案 `Nexus_92300YC_RCF_v1.0.2.txt`：
...
. 將先前對交換器組態所做的任何自訂設定重新套用。有關其他所需變更的詳細資訊，請參閱 link:cabling-considerations-92300.html["審查佈線和配置注意事項"]。
...
----
. 請確認叢集 LIF 已移轉至交換器 cs2 上的連接埠。這可能需要幾秒鐘。 `network interface show -vserver Cluster`
+
.顯示範例
[%collapsible]
====
...
----
. 重新開機交換器 cs1。這樣做是為了觸發叢集 LIF 回復到其主連接埠。在交換器重新開機期間，您可以忽略節點上報告的「叢集連接埠關閉」事件。
+
[listing, subs="+quotes"]
----
cs1# *reload*
...
====
.下一步是什麼？
安裝 RCF 後，您可以link:configure-ssh-keys.html["驗證 SSH 配置"]。
