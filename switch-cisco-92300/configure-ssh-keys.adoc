...
summary: 確認 Cisco Nexus 92300YC 叢集交換器上已啟用 SSH 和 SSH 金鑰，以支援 CSHM 和日誌收集。 
---
= 驗證 Cisco Nexus 92300YC 叢集交換器的 SSH 組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
確認 Cisco Nexus 92300YC 叢集交換器上已啟用 SSH 和 SSH 金鑰，以支援乙太網路交換器健康監視器 (CSHM) 和日誌收集功能。
...
IMPORTANT: 啟用 FIPS 時、您必須使用命令 `ssh key ecdsa 256 force`將交換器上的位元數變更為 256。如需更多詳細資料、請參閱 https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips["使用 FIPS 設定網路安全"^]。
