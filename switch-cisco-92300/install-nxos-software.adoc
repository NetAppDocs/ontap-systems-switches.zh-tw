...
summary: 在 Cisco Nexus 92300YC 叢集交換器上安裝 NX-OS 軟體，使其做好叢集運作的準備。 
---
= 在 Cisco Nexus 92300YC 叢集交換器上安裝 NX-OS 軟體
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 92300YC 叢集交換器上安裝 NX-OS 軟體，使其做好叢集運作準備。必須先安裝 NX-OS，才能設定交換器或安裝 RCF。

NX-OS 是 Cisco Systems 提供的 Nexus 系列乙太網路交換器和 MDS 系列 Fibre Channel （FC） 儲存設備區域網路交換器的網路作業系統。
...
* 適用於您交換器的 NetApp Cisco NX-OS 軟體，請從 NetApp 支援網站取得，網址為 http://mysupport.netapp.com/["mysupport.netapp.com"]
...
----
====
. 將 NX-OS 軟體和 EPLD "映像"複製到 Nexus 92300YC 交換器。
+
.顯示範例
[%collapsible]
====
...
.下一步是什麼？
安裝 NX-OS 軟體後，您可以link:install-the-rcf-file.html["安裝 RCF"]。
