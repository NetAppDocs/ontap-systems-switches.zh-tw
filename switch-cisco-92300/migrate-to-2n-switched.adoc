...
summary: 如果您已有雙節點無交換器式叢集環境，則可以使用 Cisco Nexus 92300YC 交換器移轉至雙節點有交換器叢集環境，使叢集能夠擴充至兩個以上的節點。 
---
= 移轉至使用 Cisco Nexus 92300YC 交換器的雙節點交換式叢集
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果您已有雙節點無交換器式叢集環境，可以使用 Cisco Nexus 92300YC 交換器移轉至雙節點有交換器叢集環境，讓叢集能夠擴充至兩個以上的節點。

您使用的步驟取決於每個控制器上是兩個專用的叢集網路連接埠、還是每個控制器上只有一個單一叢集連接埠。文件中所述的程序適用於所有使用光纖或雙軸連接埠的節點，但如果節點使用內建 10Gb BASE-T RJ45 連接埠作為叢集網路連接埠，則此交換器不支援該程序。

大多數系統需要每個控制器上有兩個專用的叢集網路連接埠。


NOTE: 遷移完成後、您可能需要安裝必要的組態檔、以支援 92300YC 叢集交換器的叢集交換器健全狀況監視器（ Cluster Switch Health Monitor 、 CSHM ）。請參閱 link:../switch-cshm/cshm-overview.html["交換器健全狀況監控 (CSHM)"]。
...
* 雙節點無交換器配置已正確設定並運作正常。
* 這些節點執行的是 ONTAP 9.6 及更新版本。
* 叢集所有連接埠均處於*開啟*狀態。
* 所有群集邏輯介面（LIF）均處於 *up* 狀態，並位於其主連接埠上。


Cisco Nexus 92300YC 交換器組態：

* 兩台交換器都具備管理網路連線功能。
* 可以透過控制台存取叢集交換器。
* Nexus 92300YC 節點對節點交換器與交換器對交換器的連接使用雙軸或光纖電纜。
+
link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe- 交換機"^]包含有關佈線的更多資訊。

* 交換器間連結 (ISL) 電纜連接到兩台 92300YC 交換器的連接埠 1/65 和 1/66。
* 兩台 92300YC 交換器的初始自訂已完成。因此：
+
** 92300YC 交換器執行的是最新版本的軟體
** 參考組態檔 (RCF) 應用於交換器。任何站台自訂，如 SMTP、SNMP 和 SSH，均在新交換器上進行設定。
...
----
====
. 確認兩台 92300YC 交換器 cs1 和 cs2 之間的 ISL 以及 ISL 上的實體連接埠 1/65 和 1/66 是否已啟用：
+
...
----
====
. 斷開 node1 上叢集連接埠 e0a 的纜線，然後使用 92300YC 交換器支援的適當纜線將 e0a 連接到叢集交換器 cs1 上的連接埠 1。
+
link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe- 交換機"^]包含有關佈線的更多資訊。

. 拔除節點 2 上叢集連接埠 e0a 的纜線，然後使用 92300YC 交換器支援的適當纜線將 e0a 連接至叢集交換器 cs1 的連接埠 2。
...
----
====
. 驗證所有叢集 LIF 是否都已啟動、運作正常，並且顯示為 true，適用於 `Is Home`：
+
`network interface show -vserver Cluster`
...
----
====
. 斷開節點 1 上叢集連接埠 e0b 的纜線，然後使用 92300YC 交換器支援的適當纜線將 e0b 連接到叢集交換器 cs2 的連接埠 1。
. 拔除節點 2 上叢集連接埠 e0b 的纜線，然後使用 92300YC 交換器支援的適當纜線，將 e0b 連接至叢集交換器 cs2 上的連接埠 2。
...
--
====
. [[step8]] 如果您已停用自動建立案例功能，請透過呼叫 AutoSupport 訊息重新啟用功能：
+
...
.下一步是什麼？
驗證 SSH 組態後，您可以link:../switch-cshm/config-overview.html["配置交換器健康監控"]。
