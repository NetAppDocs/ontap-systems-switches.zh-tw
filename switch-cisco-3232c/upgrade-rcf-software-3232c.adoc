...
summary: '當 Cisco Nexus 3232C 叢集交換器上有新版本可用時，請升級參考組態檔案 (RCF)。' 
---
= 升級 Cisco Nexus 3232C 叢集交換器上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 Cisco Nexus 3232C 叢集交換器上有更新版本的參考組態檔案 （RCF） 時，請升級 RCF。
...
CAUTION: 請務必關閉 *所有* 已連接的叢集連接埠，以避免任何網路連線問題。如需更多詳細資訊，請參閱知識庫文章 link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["在切換作業系統升級期間遷移叢集 LIF 時，節點脫離仲裁。"^]。

. 確認叢集連接埠已故障轉移到叢集交換器 cs1 上的連接埠。這可能需要幾秒鐘。
+
`network interface show -role cluster`
...
[NOTE]
====
請務必設定以下內容：* 使用者名稱和密碼 * 管理 IP 位址 * 預設閘道 * 交換器名稱
...
. 使用下列傳輸協定之一將 RCF 複製到交換器 cs2 的 bootflash：FTP、TFTP、SFTP 或 SCP。如需 Cisco 命令的詳細資訊，請參閱 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令參考"^] 中的適當指南。
+
.顯示範例
[%collapsible]
====
...
+
如需 Cisco 命令的詳細資訊，請參閱 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令參考"^] 中的適當指南。
...
.ONTAP 9.9.1 及更高版本
--
您可以使用 `network interface check cluster-connectivity`命令啟動叢集連接性的存取能力檢查，然後顯示詳細資訊：
...
.所有ONTAP版本
--
對於所有 ONTAP 版本，您也可以使用
`cluster ping-cluster -node <name>`命令來檢查連線能力：
