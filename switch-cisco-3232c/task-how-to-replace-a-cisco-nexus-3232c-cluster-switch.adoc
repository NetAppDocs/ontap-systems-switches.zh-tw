...
[role="lead"]
您可以執行特定的工作順序、在不中斷營運的情況下更換叢集中的故障Cisco Nexus 3232C交換器。

.開始之前
現有的叢集和網路組態必須具有下列特性：

* Nexus 3232C叢集基礎架構必須備援且在兩台交換器上都能完全正常運作。
+
「Cisco乙太網路交換器」頁面提供您交換器上最新的RCF和NX-OS版本。

* 所有叢集連接埠都必須處於「up」（開機）狀態。
* 兩台交換器都必須具備管理連線能力。
* 所有叢集邏輯介面（lifs）都必須處於「up」（啟動）狀態、而且不得移轉。
...
* *顯示叢集連接埠並將其移轉至交換器C2（步驟 <<one,1.>>-<<seven,7）>>*
* *將ISL纜線從交換器CL2重新連接到交換器C2、然後將ISL移轉到交換器CL1和C2（步驟 <<eight,8.>>-<<fourteen,14）>>*
* *將所有LIF還原至原始指派的連接埠（步驟 <<fifteen,15>>-<<eighteen,18）>>*
* *確認所有連接埠和LIF均已正確移轉（步驟 <<nineteen,19>>-<<twentyone,21>>）*


此程序會以新的3232C交換器C2取代第二個Nexus 3232C叢集交換器CL2。本程序中的範例使用下列交換器和節點命名法：
...
本程序中的範例使用四個節點。其中兩個節點使用四個10 Gb叢集互連連接埠：e0a、e0b、e0c和e0d。其他兩個節點使用兩個40 GB叢集互連連接埠：E4A和e4e。請參閱 link:https://hwu.netapp.com/SWITCH/INDEX.html["Hardware Universe"^] 驗證平台的叢集連接埠是否正確。
...
+
「網路介面移轉-vserver叢集-lif_lif-name_-source-node-node-name_-destination-node-name_-destination-port _port-name_」
...
12 entries were displayed.
----
. 關閉實體連接至原始交換器CL2的叢集互連連接埠：「network port modify -node-name_-port _port-name_-up -admin假」
+
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. 在所有節點上、開啟連接至替換交換器C2的所有叢集互連連接埠：「network port modify -node-name_-port _port-name_-up -admin true」
+
[listing]
----
...
. [[wentyon]]驗證是否監控了適當的叢集交換器：「System叢集交換器show」
+
