...
summary: 在 Cisco Nexus 3232C 叢集交換器上安裝 NX-OS 軟體，使其做好叢集運作的準備。 
---
= 在 Cisco Nexus 3232C 叢集交換器上安裝 NX-OS 軟體
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 3232C 叢集交換器上安裝 NX-OS 軟體，使其做好叢集運作的準備。
...
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco乙太網路交換器頁面"^]。請查閱交換器相容性列表，以了解支援的 ONTAP 和 NX-OS 版本。
...
. 將 NX-OS 軟體和 EPLD "映像"複製到 Nexus 3232C 交換器。
+
.顯示範例
[%collapsible]
====
...
----
====
. 如果您要升級至 NX-OS 版本 9.3(11)，則必須升級 EPLD `golden`"映像"並再次重新開機交換器。否則，請跳至步驟 12。
+
如需更多詳細資訊，請參閱 link:https://www.cisco.com/c/en/us/td/docs/switches/datacenter/nexus9000/sw/93x/epld-rn/nxos_n9K_epldRN_9311.html["EPLD Upgrade Release Notes，Release 9.3(11)"^]。
...
----
====
. 交換器重新開機後，登入以驗證新版本的 EPLD 是否已成功載入。
+
.顯示範例
[%collapsible]
====
...
.下一步是什麼？
安裝 NX-OS 軟體後，您可以link:install-upgrade-rcf-overview-3232c.html["安裝或升級參考設定檔（RCF）"]。
