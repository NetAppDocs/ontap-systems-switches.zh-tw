...
---
= 如何將CN1610叢集交換器移轉至Cisco Nexus 3232C叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
下列清單說明變更叢集交換器時必須完成的階段：

* 一將叢集交換器CL2更換為C2（步驟 <<one,1.>>-<<twentytwo,22>>）
* 二、將叢集交換器CL1更換為C1（步驟 <<twentythree,23>>-<<forty,40>>）
...
====
.步驟
. [[on]]如果AutoSupport 此叢集啟用了Section,則會叫用AutoSupport 下列訊息來抑制自動案例建立：
+
...
. 確定每個叢集介面的管理或作業狀態。
+
.. 顯示叢集網路連接埠屬性：「network port show -role cluster」
+
[listing]
----
...
8 entries were displayed.
----
.. 顯示有關邏輯介面的資訊：「network interface show -role cluster」
+
[listing]
----
...
----
. 驗證叢集的健全狀況：「network interface show -role cluster」（網路介面顯示角色叢集）
+
[listing]
----
cluster::*> network interface show -role cluster
...
+
在步驟10中、當ISL在兩台交換器上啟動時、此組態會與3232C交換器C2上的ISL組態相符。
...
----
. 驗證3232C交換器C2上的ISL是否為「up（正常）」：「How port-channel Summary（顯示連接埠通道摘要）」
+
如需Cisco命令的詳細資訊、請參閱中所列的指南 https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html["Cisco Nexus 3000系列NX-OS命令參考資料"^]。
...
2	    Po2(SU)     Eth      LACP      Eth1/24/1(P) Eth1/24/2(P) Eth1/24/3(P)
                                     Eth1/24/4(P)
----
. 開啟所有節點上連接至3232C交換器C2的所有叢集互連連接埠：「network port modify -node-name_-port _port-name_-up -admin true」
+
...
----
. 還原所有連接到所有節點上C2的所有移轉叢集互連lifs：「網路介面還原-vserver叢集-lif_lif-name_」
+
[listing]
----
...
----
. 驗證所有的叢集互連連接埠是否都已還原至其主連接埠：「network interface show -role cluster」（網路介面show -role cluster）
+
...
8 entries were displayed.
----
. 驗證是否所有的叢集連接埠都已連線：「network port show -role cluster」（網路連接埠顯示角色叢集）
+
...
8 entries were displayed.
----
. Ping遠端叢集介面、然後執行遠端程序呼叫伺服器檢查：「cluster ping cluster -node-name_」
+
下列範例顯示正在Ping節點n1、之後指出的RPC狀態：
...
4 paths up, 0 paths down (udp check)
----
. [[wenty2]]移轉與第一個CN1610交換器CL1相關聯的LIF：「網路介面移轉-vserver叢集-lif_lif-name_-source-node-name_」
+
...
----
. [[[thentyther]]驗證叢集的狀態：「network interface show -role cluster」（網路介面顯示角色叢集）
+
...
8 entries were displayed.
----
. 關閉所有節點上連接至CL1的節點連接埠：「network port modify -node-name_-port _port-name_-up -admin假」
+
...
1     Po1(SU)     Eth      LACP      Eth1/31(P)   Eth1/32(P)
----
. 將所有連接至所有節點上新3232C交換器C1的叢集互連連接埠全部開機：「network port modify -node-name_-port _port-name_-up -admin true」（網路連接埠修改-node-name_-port _port-name_-up admin true）
+
...
----
. 驗證叢集節點連接埠的狀態：「network port show -role cluster」（網路連接埠show -role cluster）
+
...
8 entries were displayed.
----
. 還原所有原本連線至所有節點C1的移轉叢集互連生命體：「網路介面還原-伺服器叢集-lif_lif-name_」
+
您必須個別移轉每個LIF、如下列範例所示：
...
----
. 驗證介面現在是主介面：「網路介面show -role cluster」（網路介面show -role叢集）
+
...
8 entries were displayed.
----
. Ping遠端叢集介面、然後執行遠端程序呼叫伺服器檢查：「cluster ping cluster -node-host-name_」
+
下列範例顯示正在Ping節點n1、之後指出的RPC狀態：
...
** 「系統叢集交換器顯示」+下列範例顯示節點n3和n4、其中40個GbE叢集連接埠分別連接至Nexus 3232C叢集交換器上的連接埠E1/7和E1/8。這兩個節點都會加入叢集。使用的40 GbE叢集互連連接埠為E4A和e4e。
...
----
. 如果更換的CN1610交換器未自動移除、請將其移除：「系統叢集交換器刪除裝置_switch-name_」
+
...
----
. [[40：]啟用叢集交換器健全狀況監視器記錄收集功能、以收集交換器相關的記錄檔：「系統叢集交換器記錄設定密碼」
+
「系統叢集交換器記錄啟用收集」
