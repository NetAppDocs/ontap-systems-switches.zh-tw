...
---
= 如何從Cisco Nexus 5596叢集交換器移轉至Cisco Nexus 3232C叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
若要使用Nexus 3232C叢集交換器取代叢集中現有的Cisco Nexus 5596叢集交換器、您必須執行特定的工作順序。

.關於這項工作
本程序的範例說明如何將Cisco Nexus 5596交換器更換為Cisco Nexus 3232C交換器。您可以針對其他舊版Cisco交換器（例如、31382Q-V）使用這些步驟（進行修改）。此程序也使用下列交換器和節點命名法：
...
[NOTE]
====
本程序的範例使用四個節點：兩個節點使用四個10 GbE叢集互連連接埠：e0a、e0b、e0c和e0d。其他兩個節點使用兩個40 GbE叢集互連連接埠：E4A、e4e。。 link:https://hwu.netapp.com/["_SUR__ Hardware Universe"^] 列出您平台上的實際叢集連接埠。
...
* 叢集已新增兩個FAS9000節點、其中的範例顯示叢集詳細資料（步驟34至37）。
...
+
「網路介面移轉-vserver叢集-lif_lif-name_-source-node-node-name_–destination-node-node-name_-destination-port _destination-port-name_」
...
----
. 在所有節點上、開啟連接至新款3232C交換器C1：「網路連接埠修改」的所有叢集互連連接埠
+
...
----
. 使用「系統叢集交換器刪除」命令移除已更換的Nexus 5596、如果未自動移除：「系統叢集交換器刪除裝置交換器名稱」
+
[listing]
----
