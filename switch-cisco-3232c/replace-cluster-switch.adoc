...
summary: 更換發生故障的 Cisco Nexus 3232C 叢集交換器。此程序不會中斷營運。 
---
= 更換 Cisco Nexus 3232C 叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中斷叢集運作的情況下更換故障的 Cisco Nexus 3232C 叢集交換器。此不中斷營運的程序包括準備替換交換器，並將連線從故障交換器移轉過來。
...
* Nexus 3232C 叢集基礎架構是備援的，並且在兩台交換器上都能完全正常運作。
+
Cisco乙太網路交換器頁麵包含交換器上最新的 RCF 和 NX-OS 版本。

* 叢集中的所有連接埠必須處於*啟用*狀態。
* 兩台交換器都必須具備管理連線功能。
* 所有叢集邏輯介面（LIF）均處於 *up* 狀態，且未移轉。


替換用的 Cisco Nexus 3232C 交換器具有以下特點：
...
* 您計劃將叢集交換器 CL2 更換為 C2（步驟 1 至 21）：
+
** 在每個節點上，將連接到叢集交換器 CL2 的叢集 LIF 移轉到連接到叢集交換器 CL1 的叢集連接埠。
** 中斷叢集交換器 CL2 上所有連接埠的纜線，然後將纜線重新連接到替換叢集交換器 C2 上的相同連接埠。
** 您回復每個節點上已移轉的叢集 LIF。
...
.關於範例
此更換程序將第二個 Nexus 3232C 叢集交換器 CL2 更換為新的 3232C 交換器 C2。
...
. 確認新的 Nexus 3232C 交換器上已安裝對應的 RCF 和映像，並進行任何必要的站台自訂。
+
.. 前往 NetApp 支援網站。
+
http://mysupport.netapp.com/["mysupport.netapp.com"^]

.. 前往 *Cisco 乙太網路交換器* 頁面，並記下表格中所需的軟體版本。
+
https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco乙太網路交換機"^]

.. 下載適當版本的 RCF。
.. 在*描述*頁面上按一下*繼續*，接受授權合約，然後瀏覽至*下載*頁面。
.. 從 *Cisco® 叢集和管理網路交換器參考組態檔下載* 頁面下載正確版本的映像軟體。
+
...
. 將叢集 LIF 移轉至連接至替換交換器 C2 的實體節點連接埠：
+
...
----
====
. 拆下連接到叢集交換器 CL2 的所有纜線，並將其重新連接到所有節點的替換交換器 C2。
...
+
連接埠 Eth1/31 和 Eth1/32 應顯示 `(P)`，這表示 ISL 連接埠已在連接埠通道中啟動：
...
----
====
. 在所有節點上，啟動連接到替換交換器 C2 的所有叢集互連連接埠：
+
...
=== 步驟 3：將所有 LIF 回復至原先指派的連接埠

. 還原所有節點上所有已移轉的叢集互連 LIF：
+
...
[%collapsible]
====
以下範例顯示所有 LIF 已成功回復，因為  `Current Port` 欄下所列的連接埠在  `Is Home` 欄中的狀態為  `true`。如果連接埠的值為  `false`，則 LIF 尚未回復。
...
[%collapsible]
====
以下範例表示叢集交換器受到監控，因為其 `Is Monitored`狀態為 `true`。
