...
summary: 在不中斷系統運作的情況下，更換 AFX 系統中發生故障的 Cisco Nexus 9332D-GX2B 交換器。這是不中斷營運的程序。 
---
= 更換 AFX 系統中的 Cisco Nexus 9332D-GX2B 交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中斷系統運作的情況下，更換 AFX 系統中發生故障的 Cisco Nexus 9332D-GX2B 交換器。此不中斷營運的操作 (NDU) 包括準備替換交換器和還原系統組態。
...
** 交換器的初始自訂已完成，詳情請見 link:configure-complete-initial-setup-9332d.html["完成初始交換器設置"]。
+
...
* 在維護前後觸發維護 AutoSupport，以在維護期間停用案例建立功能。如需更多詳細資訊，請參閱此知識庫文章 https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU92["SU92：如何在計劃維護視窗期間抑制自動建立案例"^]。
...
----
====
. 啟用 newcs2 交換器上的所有連接埠，從 Eth1/1 到 Eth1/30。
+
.顯示範例
[%collapsible]
====
...
. 驗證遠端集群介面的連接性：
+
.. 你可以使用 `network interface check cluster-connectivity`用於顯示叢集連接性可訪問性檢查詳細資訊的命令：
+
[source, cli]
----
