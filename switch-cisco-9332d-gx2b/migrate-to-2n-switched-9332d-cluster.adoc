...
summary: 如果您已有雙節點無交換器叢集環境，則可以使用Cisco Nexus 9332D-GX2B 交換器遷移到雙節點有交換器叢集環境，以便能夠將叢集擴展到兩個以上節點。 
---
= 遷移到雙節點交換集群
:allow-uri-read: 
:icons: font
...
[role="lead"]
如果您已有雙節點無交換器叢集環境，則可以使用Cisco Nexus 9332D-GX2B 交換器遷移到雙節點有交換器叢集環境。
...
* 對於Cisco Nexus 9332D-GX2B 交換器配置：
+
** 兩台交換器都具備管理網路連線功能。
** 可以透過控制台存取叢集交換器。
** Nexus 9332D-GX2B 節點到節點交換器和交換器到交換器連接使用 Twinax 或光纖電纜。
+
看 https://hwu.netapp.com["Hardware Universe"^]有關佈線的更多資訊。



* 交換器間連結 (ISL) 電纜連接到兩個 9332D-GX2B 交換器上的連接埠 1/35 和 1/36。
* 兩台 9332D-GX2B 交換器的初始自訂已完成，因此：
+
** 9332D-GX2B 交換器運作的是最新版本的軟體。
...
看 https://hwu.netapp.com["Hardware Universe"^]有關您平台集群連接埠的資訊。看 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^] 有關交換器安裝要求的詳細資訊。
...
----
====
. 驗證兩個 9332D-GX2B 交換器 cs1 和 cs2 之間的 ISL 以及 ISL 上的實體連接埠在連接埠 1/35 和 1/36 上是否處於啟動狀態：
+
...
----
====
. 斷開節點 1 上的叢集連接埠 e0a 的電纜，然後使用 9332D-GX2B 交換器支援的適當電纜將 e0a 連接到叢集交換器 cs1 上的連接埠 1。
+
...
. 斷開節點 2 上的叢集連接埠 e0a 的電纜，然後使用 9332D-GX2B 交換器支援的適當電纜將 e0a 連接到叢集交換器 cs1 上的連接埠 2。
...
----
====
. 斷開節點 1 上的叢集連接埠 e0b 的電纜，然後使用 9332D-GX2B 交換器支援的適當電纜將 e0b 連接到叢集交換器 cs2 上的連接埠 1。
. 斷開節點 2 上的叢集連接埠 e0b 的電纜，然後使用 9332D-GX2B 交換器支援的適當電纜將 e0b 連接到叢集交換器 cs2 上的連接埠 2。
