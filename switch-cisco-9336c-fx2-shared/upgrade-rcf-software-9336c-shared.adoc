...
summary: 當 Cisco Nexus 9336C-FX2 共享交換器上有更新版本時，請升級參考組態檔案 （RCF）。 
---
= 升級 Cisco Nexus 9336C-FX2 共享交換器上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 Cisco Nexus 9336C-FX2 共享交換器上有更新版本的參考組態檔案 （RCF） 可用時，請升級 RCF。
...
----
====
. 檢查每個叢集連接埠、節點儲存連接埠和儲存架連接埠的管理和運作狀態。
+
.. 確認叢集所有連接埠均已啟動且狀態正常：
+
...
+
.. 記錄目前運行配置和正在使用的 RCF 檔案之間的任何自訂新增內容（例如，貴組織的 SNMP 配置）。
...
. 有關任何後續更改的詳細資訊，請參閱 link:cabling-considerations-9336c-shared.html["審查佈線和配置注意事項"]。
...
====
.下一步是什麼？
升級 RCF 後，您可以 link:../switch-cshm/config-overview.html["配置交換器健康監控"]。
