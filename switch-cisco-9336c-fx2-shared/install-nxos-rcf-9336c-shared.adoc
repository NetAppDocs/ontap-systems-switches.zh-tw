...
summary: '在 Cisco Nexus 9336C-FX2 共享交換器上完成初始設定後，安裝參考組態檔案 (RCF)，以設定合併叢集與儲存設備作業的連接埠指派。' 
---
= 在Cisco Nexus 9336C-FX2 共用交換器上安裝 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成初始設定後，在 Cisco Nexus 9336C-FX2 共享的交換器上安裝參考組態檔案 （RCF）。RCF 可設定叢集與儲存設備作業的連接埠指派。
