...
大多數系統在每個控制器上都需要兩個專用的叢集網路連接埠。請參閱  https://["Cisco乙太網路交換器"] 以取得更多資訊。
...
** NetApp https://["Hardware Universe"] 包含纜線的詳細資訊。
...
* cluster1：：*>提示會指出叢集的名稱。
* 此程序中使用的叢集連接埠是_e3a_和_e3c_、如同AFF 根據ESIA400控制器。。 https://["Hardware Universe"] 包含平台實際叢集連接埠的最新資訊。
