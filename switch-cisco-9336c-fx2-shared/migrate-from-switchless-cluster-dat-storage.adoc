...
keywords:  
summary: 透過新增兩個新的共享交換器，將無交換器式叢集（採用直連式儲存設備）移轉至 Cisco Nexus 9336C-FX2 共享交換器。 
---
= 將採用直連式儲存設備的無交換器式叢集移轉至 Cisco Nexus 9336C-FX2 共享的交換器
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
透過新增兩個新的共享交換器，將無交換器式叢集（採用直連式儲存設備）移轉至 Cisco Nexus 9336C-FX2 共享交換器。

具體操作步驟取決於每個控制器上是有兩個專用叢集網路連接埠還是只有一個叢集連接埠。記錄的過程適用於所有使用光纖或 Twinax 連接埠的節點，但如果節點使用板載 10Gb BASE-T RJ45 連接埠作為叢集網路端口，則此交換器不支援此流程。

大多數系統需要每個控制器上有兩個專用的叢集網路連接埠。如需詳細資訊，請參閱 link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco乙太網路交換機"^]。
...
** link:https://hwu.netapp.com["Hardware Universe"^]包含有關佈線的更多資訊。
...
* 根據 AFF A400 控制器，此程序中使用的叢集連接埠為 _e3a_ 和 _e3b_。link:https://hwu.netapp.com["Hardware Universe"^]包含有關您平台實際叢集連接埠的最新資訊。
...
+
link:https://hwu.netapp.com["Hardware Universe"^] 包含更多關於佈線的資訊。如需交換器安裝需求的詳細資訊，請參閱 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^]。
