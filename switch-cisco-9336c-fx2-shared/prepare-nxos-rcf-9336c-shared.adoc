...
keywords:  
summary: '在安裝 NX-OS 軟體和參考組態檔案 (RCF) 之前，請準備好 Cisco Nexus 9336C-FX2 共享的交換器。' 
---
= 準備在 Cisco Nexus 9336C-FX2 共享交換器上安裝 NX-OS 軟體和 RCF
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在安裝 NX-OS 軟體和參考組態檔案（RCF）之前，請先準備好 Cisco Nexus 9336C-FX2 共享的交換器。必須先安裝 NX-OS 和 RCF，交換器才能在 ONTAP 環境中運作。
...
----
====
. 檢查每個叢集介面、每個節點儲存連接埠和儲存架連接埠的管理或運作狀態。
+
.. 顯示網路連接埠屬性：
+
[source, cli]
...
.下一步是什麼？
準備好安裝 NX-OS 軟體和 RCF 後，您可以… link:install-nxos-software-9336c-shared.html["安裝 NX-OS 軟體"]。
