...
keywords:  
summary: 透過新增兩台新的共享交換器，將採用直連式儲存設備的交換式組態移轉至 Cisco Nexus 9336C-FX2 共享交換器。 
---
= 將採用直連式儲存設備的交換式組態移轉至 Cisco Nexus 9336C-FX2 共享的交換器
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
透過新增兩台新的共享交換器，將採用直連式儲存設備的交換式組態移轉至 Cisco Nexus 9336C-FX2 共享交換器。
...
本流程支援的 ONTAP 和 NX-OS 版本位於 Cisco 乙太網路交換器頁面。請參閱 https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco乙太網路交換機"^]。
...
link:https://hwu.netapp.com["Hardware Universe"^] 包含所有叢集交換器支援的纜線資訊。
