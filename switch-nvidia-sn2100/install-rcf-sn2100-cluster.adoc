...
summary: '在 NVIDIA SN2100 叢集交換器上安裝或升級參考組態檔案 (RCF) 指令碼。' 
---
= 在 NVIDIA SN2100 叢集交換器上安裝或升級 RCF 指令碼
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 NVIDIA SN2100 叢集交換器上安裝或升級參考組態檔案 (RCF) 指令碼，以設定連接埠指派和網路設定。

.開始之前
在安裝或升級 RCF 腳本之前，請確保交換器上具備以下條件：

* Cumulus Linux 已安裝。請參閱 link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] 以了解支援的版本。
...
| 簇 | 在設定為 4x10GbE 分支的連接埠上，一個連接埠設定為 4x25GbE 分支，其他連接埠則設定為 40/100GbE。支援使用共享叢集 / HA 連接埠之節點上的共享叢集 / HA 流量。請參閱知識庫文章中的平台表格 link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["哪些AFF、 ASA和FAS平台使用共用叢集和 HA 乙太網路連接埠？"^]。所有連接埠也可做為專用叢集連接埠使用。 
...
--
. 將集群交換器連接到管理網路。
. 使用 `ping`用於驗證與託管 Cumulus Linux 和 RCF 的伺服器的連接性的命令。
...
NOTE: 儘管 `scp`如果範例中使用的是這種方式，您可以使用您喜歡的檔案傳輸方式，例如 SFTP、HTTPS 或 FTP。
...
NOTE: 在上述步驟 3「更新 MOTD 檔案」中，該指令 `cat /etc/issue.net`正在運行。這樣，您就可以驗證 RCF 檔案名稱、RCF 版本、要使用的連接埠以及 RCF 橫幅中的其他重要資訊。
...
NOTE: 雖然 `scp`如果範例中使用的是這種方式，您可以使用您喜歡的檔案傳輸方式，例如 SFTP、HTTPS 或 FTP。
...
NOTE: 在上述步驟 3 *更新 MOTD 檔案* 中，執行指令 `cat /etc/issue.net`。這樣，您就可以驗證 RCF 檔案名稱、RCF 版本、要使用的連接埠以及 RCF 橫幅中的其他重要資訊。
...
====
.下一步是什麼？
安裝 RCF 後，您可以… link:setup-install-cshm-file.html["安裝 CSHM 文件"]。
