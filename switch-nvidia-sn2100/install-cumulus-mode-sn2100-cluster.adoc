...
summary: 在 NVIDIA SN2100 叢集交換器上安裝 Cumulus Linux，前提是交換器運作在 Cumulus 模式下。 
---
= 在 NVIDIA SN2100 叢集交換器上以 Cumulus 模式安裝 Cumulus Linux
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 NVIDIA SN2100 叢集交換器以 Cumulus 模式運作時，在交換器上安裝 Cumulus Linux （CL）。


NOTE: 當交換器執行 Cumulus Linux 或 ONIE 時、可以安裝 Cumulus Linux（CL）作業系統（請參閱 link:install-onie-mode-sn2100-cluster.html["以 ONIE 模式安裝"]）。
...
----
+
如需詳細資訊，請參閱 https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-54/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA使用者帳戶"^]。
...
如需詳細資訊，請參閱 https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA使用者帳戶"^]。

--
====
.下一步是什麼？
在 Cumulus 模式下安裝 Cumulus Linux 後，您 link:install-rcf-sn2100-cluster.html["安裝參考設定檔 (RCF) 腳本"]。
