...
summary: 在不中斷儲存網路運作的情況下，將舊款 Cisco 儲存交換器移轉至 NVIDIA SN2100 儲存交換器。 
---
= 將儲存設備交換器從 Cisco 移轉至 NVIDIA SN2100 儲存設備交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中斷儲存網路運作的情況下，將舊款 Cisco 儲存交換器移轉至 NVIDIA SN2100 儲存交換器。
...
當您用 NVIDIA SN2100 儲存交換器取代一些老舊的 Cisco 交換器時、必須了解某些組態資訊、連接埠連線和佈線要求。請參閱 link:configure-overview-sn2100-cluster.html["NVIDIA SN2100交換器的安裝與設定概述"]。
...
* NVIDIA SN2100 交換器上的某些連接埠配置為以 100 GbE 運作。
...
----
====
. 從節點的角度來看，每個節點上的儲存連接埠透過以下命令連接到現有的儲存交換器：
+
...
----
====
. 在交換器 S1 和 S2 上，儲存連接埠和交換器透過以下命令連接（從交換器的角度來看）：
+
`show lldp neighbors`
...
----
====
. 從節點的角度來看，每個節點上的儲存連接埠現在會以以下方式連接到交換器：
+
.顯示範例
[%collapsible]
====
