...
當您將一些老舊的 Cisco 叢集交換器替換為 NVIDIA SN2100 叢集交換器時，必須了解某些組態資訊、連接埠連線和佈線要求。請參閱 link:configure-overview-sn2100-cluster.html["NVIDIA SN2100交換器的安裝與設定概述"]。
...
有關支援的連接埠及其組態的詳細資訊，請參閱 link:https://hwu.netapp.com/["Hardware Universe"^] 。
...
NOTE: 如果您變更 AFF A800 或 AFF C800 系統上 e0a 和 e1a 叢集連接埠的連接埠速率、則可能會在速率轉換後發現接收到的封包格式錯誤。請參閱 link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1570339["漏洞編號 1570339"^] 和知識庫文章 link:https://kb.netapp.com/onprem/ontap/hardware/CRC_errors_on_T6_ports_after_converting_from_40GbE_to_100GbE["將 40GbE 轉換為 100GbE 後，T6 連接埠出現 CRC 錯誤"^] 以取得指引。
