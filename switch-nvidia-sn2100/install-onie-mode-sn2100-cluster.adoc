...
summary: 在 NVIDIA SN2100 叢集交換器上安裝 Cumulus Linux （前提是交換器在 ONIE 模式下運作）。 
---
= 在 NVIDIA SN2100 叢集交換器上以 ONIE 模式安裝 Cumulus Linux
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 NVIDIA SN2100 叢集交換器以 ONIE 模式運作時，在交換器上安裝 Cumulus Linux （CL）。


NOTE: Cumulus Linux （ CL ） OS 可以在交換器執行 ONIE 或 Cumulus Linux 時安裝（請參閱 link:install-cumulus-mode-sn2100-cluster.html["以 Cumulus 模式安裝"]）。
