...
summary: ONTAP 中的乙太網路交換器健全狀況監控器 CSHM 記錄收集功能用於從叢集或儲存網路交換器收集記錄。 
---
= 配置日誌收集
:allow-uri-read: 
:icons: font
...
[role="lead"]
乙太網路交換器健康狀況監控器（CSHM）負責確保網路交換器的運作健全狀況，並收集交換器日誌以進行偵錯。本程序將引導您完成設定收集、請求詳細的 *Support* 日誌，以及啟用每小時收集 AutoSupport 所收集之 *Periodic* 資料的程序。
