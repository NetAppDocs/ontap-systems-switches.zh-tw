...
summary: 在網路交換器上設定 SNMPv3，以啟用與乙太網路交換器健全狀況監視器 （CSHM） 的安全的通訊。 
---
= 設定 SNMPv3 以監控乙太網路交換器的健全狀況
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在網路交換器上設定 SNMPv3，以啟用與乙太網路交換器健全狀況監視器 （CSHM） 的安全的認證通訊。SNMPv3 提供加密的交換器狀態輪詢，用於 ONTAP 中的健全狀況監控。

乙太網路交換器健康狀況監控器（CSHM）利用 SNMP 來監控網路交換器的健全狀況和效能。預設情況下，SNMPv2c 會透過參考組態檔（RCF）自動設定。SNMPv3 比 SNMPv2 更安全，因為它引入了強大的安全功能，例如驗證、加密和訊息完整性，可防止未經授權的存取，並確保傳輸期間資料的機密性和完整性。
...
+
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-82891["對於使用ONTAP對Cisco交換器進行健康監控的情況，即使切換到 SNMPv3 進行監控，可能仍會看到 SNMPv2 流量。"^]
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-119131["當 SNMP 故障發生時，交換器風扇和電源警報可能會出現誤報。"^]
...
----
. 等待 CSHM 輪詢週期結束後，確認乙太網路交換器的序號已填入。
+
[source, cli]
----
...
.Cisco交換機
--
在Cisco 9336C-FX2 交換器上設定 SNMPv3 使用者名稱 SNMPv3_USER：
...
.NVIDIA - CL 5.4.0
--
在執行 CLI 5.4.0 的NVIDIA SN2100 交換器上設定 SNMPv3 使用者名稱 SNMPv3_USER：
...
.NVIDIA - CL 5.11.0
--
在執行 CLI 5.11.0 的NVIDIA SN2100 交換器上設定 SNMPv3 使用者名稱 SNMPv3_USER：
