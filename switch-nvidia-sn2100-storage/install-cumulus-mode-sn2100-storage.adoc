...
summary: 在 NVIDIA SN2100 儲存設備交換器以 Cumulus 模式運作時，在其上安裝 Cumulus Linux。 
---
= 在 NVIDIA SN2100 儲存交換器上以 Cumulus 模式安裝 Cumulus Linux
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 NVIDIA SN2100 儲存交換器以 Cumulus 模式運作時，在交換器上安裝 Cumulus Linux （CL）。


NOTE: 當交換器執行 Cumulus Linux 或 ONIE 時、可以安裝 Cumulus Linux（CL）作業系統（請參閱 link:install-onie-mode-sn2100-storage.html["以 ONIE 模式安裝"]）。
...
----
+
如需更多詳細資料、請參閱知識庫文章 link:https://kb.netapp.com/on-prem/Switches/Nvidia-KBs/NTP_Server_configuration_is_not_working_with_NVIDIA_SN2100_Switches["NTP 伺服器設定與NVIDIA SN2100 交換器不相容。"^]。
...
----
+
如需更多資訊，請參閱 link:https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-54/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA使用者帳戶"^]。
...
如需更多資訊，請參閱 link:https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA使用者帳戶"^]。
