...
summary: 更換故障的 NVIDIA SN2100 儲存網路交換器。此為不中斷營運的程序。 
---
= 更換 NVIDIA SN2100 儲存交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中斷儲存網路運作的情況下更換故障的 NVIDIA SN2100 儲存網路交換器。此不中斷營運的程序包括準備替換交換器和還原儲存網路組態。

.開始之前
在 NVIDIA SN2100 儲存交換器上安裝 Cumulus 軟體和 RCF 之前，請確認：

* 您的系統可以支援NVIDIA SN2100 儲存交換器。
* 您已下載適用的 RCF 檔案。


link:http://hwu.netapp.com["Hardware Universe"^]提供所支援連接埠及其組態的完整詳細資料。
