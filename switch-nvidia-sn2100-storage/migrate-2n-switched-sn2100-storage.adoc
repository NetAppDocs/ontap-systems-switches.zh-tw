...
summary: 將現有的雙節點無交換器式叢集移轉至使用 NVIDIA SN2100 叢集交換器的交換式叢集環境。 
---
...
[role="lead"]
使用 NVIDIA SN2100 叢集交換器，將現有的雙節點無交換器式叢集移轉至有交換器叢集環境。這種不中斷營運的移轉可讓您將叢集擴充至兩個節點以上。
...
NOTE: 有關注意事項和更多詳情，請參閱 link:cabling-considerations-sn2100-storage.html["佈線和配置注意事項"]。
