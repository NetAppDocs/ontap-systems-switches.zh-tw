...
---
= 如何使用Cisco Nexus 9336C-FX2叢集交換器移轉至交換式NetApp叢集環境
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
----
. 驗證ISL和ISL上兩個9336C-FX2交換器CS1和CS2之間的實體連接埠是否在連接埠1/35和1/36上：「How port-channel Summary（顯示連接埠通道摘要）」
+
...
----
. 驗證所有叢集生命體是否都正常運作、並顯示為「is Home」（原為主目錄）：「network interface show -vserver Cluster」（網路介面show -vserver叢集）
+
...
4 entries were displayed.
----
. 驗證所有介面是否都顯示為「is Home」（原為主介面）：「network interface show -vserver Cluster（網路介面show -vserver叢集）」
+
...
Total entries displayed: 4
----
. 顯示叢集中探索到的網路裝置相關資訊：「network device-dDiscovery show -protocol cup」
+
[listing]
----
