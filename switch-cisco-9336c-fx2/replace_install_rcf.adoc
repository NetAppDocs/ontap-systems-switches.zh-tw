...
----
. 確認叢集連接埠已移轉至叢集交換器CS1上裝載的連接埠。這可能需要幾秒鐘的時間。'network interface show -role cluster（網路介面顯示角色叢集）'
+
[listing]
----
cluster1::*> network interface show -role cluster
...
NOTE: 您可能會在CS1交換器主控台觀察到下列輸出、視先前載入交換器的RCF版本而定：
...
.
----
. 驗證CS1和CS2之間的ISL是否正常運作：「show port-channel Summary」（顯示連接埠通道摘要）
+
[listing]
----
