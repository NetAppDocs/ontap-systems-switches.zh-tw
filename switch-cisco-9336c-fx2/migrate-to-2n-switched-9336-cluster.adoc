...
summary: 使用 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 叢集交換器，將現有的雙節點無交換器式叢集移轉至交換式叢集環境。 
---
= 使用 Cisco Nexus 9336C-FX2 叢集交換器移轉至雙節點交換式叢集
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 叢集交換器，將現有的雙節點無交換器式叢集移轉至有交換器的叢集環境。此不中斷營運的移轉可讓您將叢集擴充至兩個節點以上。
...
請參閱 link:https://hwu.netapp.com["Hardware Universe"^] 以取得平台叢集連接埠的相關資訊。請參閱 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^] 以取得交換器安裝需求的詳細資訊。
...
[%collapsible]
====
以下範例顯示 node1 和 node2 上的所有 LIF 都已啟動：
