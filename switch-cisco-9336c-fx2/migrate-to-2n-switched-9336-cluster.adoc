...
[role="lead"]
如果您現有的雙節點_無交換器_叢集環境、可以使用Cisco Nexus 9336C-FX2交換器移轉至雙節點_交換_叢集環境。
...
** 請參閱 https://["Hardware Universe"^] 適用於所有支援ONTAP 的版本。
...
+
請參閱 https://["Hardware Universe"^] 如需纜線的詳細資訊、
...
請參閱 https://["Hardware Universe"^] 以取得平台叢集連接埠的相關資訊。
...
+
。 https://["交換器Hardware Universe"^] 包含纜線的詳細資訊。

+
https://["交換器Hardware Universe"^]
