...
summary: 當有新版本可用時，請升級 Cisco Nexus 3132Q-V 叢集交換器上的參考組態檔案 （RCF）。 
---
= 升級 Cisco Nexus 3132Q-V 叢集交換器上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 Cisco Nexus 3132Q-V 叢集交換器上有更新版本的參考組態檔案 （RCF） 時，請升級 RCF。
...
. 記錄目前執行中組態與正在使用的 RCF 檔案之間的任何自訂新增內容。
+
[NOTE]
====
確保配置以下內容：
...
+
有關 Cisco 命令的更多資訊，請參閱  https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令參考"^] 中的相應指南。
...
NOTE: 有關 RCF 升級後如何使 10GbE 連接埠上線的步驟、請參閱知識庫文章 link:https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["Cisco 3132Q 叢集交換器上的 10GbE 連接埠無法上線"^]。



. 確認 RCF 版本和交換器設定正確後，將 `running-config`檔案複製到 `startup-config`檔案。
+
如需 Cisco 命令的詳細資訊，請參閱 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令參考"^] 中的適當指南。顯示範例
...
----
====
. 重新開機交換器 cs2。您可以忽略交換器重新開機期間節點上報告的「叢集連接埠關閉」事件和錯誤 `% Invalid command at '^' marker`輸出。
+
[listing, subs="+quotes"]
----
cs2# *reload*
This command will reboot the system. (y/n)?  [n] *y*
----
. 將先前對交換器組態所做的任何自訂設定重新套用。有關其他所需變更的詳細資訊，請參閱 link:cabling-considerations-3132q-v.html["審查佈線和配置注意事項"]。
. 檢查叢集上叢集連接埠的運作狀況。
+
