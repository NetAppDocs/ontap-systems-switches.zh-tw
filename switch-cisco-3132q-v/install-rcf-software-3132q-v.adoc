...
summary: 在 Cisco Nexus 3132Q-V 叢集交換器上完成初始設定後，安裝參考組態檔案 （RCF）。 
---
= 在 Cisco Nexus 3132Q-V 叢集交換器上安裝 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成初始設定後，在 Cisco Nexus 3132Q-V 叢集交換器上安裝參考組態檔案 （RCF）。RCF 定義叢集運作所需的連接埠分配和網路設定。
...
+
如需更多詳細資訊，請參閱 link:https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["如何在保持遠端連線的情況下清除Cisco互連交換器上的配置"^]。
...
請參閱 link:https://docs.netapp.com/us-en/ontap/task_configure_ontap.html["使用 System Manager 在新叢集上設定ONTAP"] 以取得設定說明。
