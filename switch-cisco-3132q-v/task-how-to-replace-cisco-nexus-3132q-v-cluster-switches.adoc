...
[role="lead"]
在叢集網路中更換故障的Cisco Nexus 3132Q-V交換器是不中斷營運的程序（NDO）、您必須執行特定的工作順序。

.開始之前
* 現有的叢集和網路組態必須具備：
+
** Nexus 3132Q-V叢集基礎架構必須備援且能在兩台交換器上完全正常運作。
+
...
** 更換交換器的主控台存取必須已就緒。
** 所需的RCF和NX-OS作業系統映像交換器必須載入至交換器。
** 交換器的初始自訂必須完成。




此程序將第二個Nexus 31382Q-V叢集交換器CL2取代為全新的31382Q-V交換器C2。本程序中的範例使用下列交換器和節點命名法：
...
* 叢集交換器CL2將由C2（<<step1_replace3132q,步驟1>> -<<step21_replace3132q,21>>）
+
...
.步驟
. [[step1_replace31382q]]如果AutoSupport 此叢集已啟用Section,請叫用AutoSupport 一個消息來抑制自動建立個案：
+
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. 在所有節點上、開啟連接至Nexus 3132Q-V交換器C2的所有叢集互連連接埠：「網路連接埠修改」
+
[listing]
----
...
----
. [[step21_replace31382q]移除已更換的Nexus 3132Q-V交換器（如果尚未自動移除）：
+
「系統叢集交換器刪除」
