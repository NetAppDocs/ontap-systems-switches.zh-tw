...
---
= 使用無交換器連接取代Cisco Nexus 3132Q-V 叢集交換機
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在ONTAP 9.3 及更高版本中，您可以將叢集從具有交換叢集網路的叢集遷移到兩個節點直接連接的叢集。

[NOTE]
====
NetApp建議您在對Cisco Nexus 3132Q-V 交換器進行從交換式叢集到無交換式叢集操作之前，更新您的ONTAP版本。

請參閱以下內容以了解更多詳細資訊：

* link:https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU540["SU540：從 40G 網路交換器升級到 100G 網路交換器時，Chelsio T6 網卡錯誤導致系統關機"^]
* link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_panic_after_migration_from_switched_to_switchless_cluster["從有交換器叢集遷移到無交換器叢集後發生節點崩潰"^]
