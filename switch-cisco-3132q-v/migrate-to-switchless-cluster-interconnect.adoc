...
---
= 以無交換器連線取代Cisco Nexus 3132Q-V叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 ONTAP 9.3 及更高版本中，您可以從具有交換叢集網路的叢集遷移到兩個節點直接連接的叢集。

[NOTE]
====
NetApp 建議您在繼續對 Cisco Nexus 3132Q-V 交換器進行切換到無交換器叢集操作之前更新 ONTAP 版本。

如需詳細資訊、請參閱下列內容：

* link:https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU540["SU540：從 40G 升級到 100G 網路交換器時，Chelsio T6 NIC 錯誤導致系統關閉"^]
* link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_panic_after_migration_from_switched_to_switchless_cluster["從交換叢集遷移到無交換叢集後節點崩潰"^]
