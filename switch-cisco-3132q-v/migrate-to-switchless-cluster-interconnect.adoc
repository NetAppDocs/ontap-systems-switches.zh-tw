...
summary: 使用無交換器式連接取代 Cisco Nexus 3132Q-V 叢集交換器，以支援 ONTAP 9.3 及更新版本。 
---
= 用無交換器連接取代 Cisco Nexus 3132Q-V 叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 ONTAP 9.3 及更新版本中，透過將 Cisco Nexus 3132Q-V 叢集交換器替換為直接連線，將具有交換式互連的叢集移轉至兩個節點直接連線的叢集。

[NOTE]
====
NetApp 建議您在對 Cisco Nexus 3132Q-V 交換器進行有交換器叢集到無交換器式叢集作業之前，先更新您的 ONTAP 版本。

如需詳細資訊，請參閱下列內容：

* link:https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU540["SU540：從 40G 升級至 100G 網路交換器時，Chelsio T6 NIC 錯誤導致系統關機"^]
* link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_panic_after_migration_from_switched_to_switchless_cluster["從有交換器叢集移轉到無交換器式叢集後發生節點崩潰"^]
