...
summary: 更換發生故障的 Cisco Nexus 3132Q-V 叢集交換器。此程序不會中斷營運。 
---
= 更換 Cisco Nexus 3132Q-V 叢集交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中斷叢集運作的情況下更換故障的 Cisco Nexus 3132Q-V 叢集交換器。此不中斷營運的程序 （NDU） 包括準備替換交換器，並將連線從故障交換器移轉過來。
...
.交換器需求
請檢閱link:switch-requirements.html["更換 Cisco Nexus 3132Q-V 叢集交換器的需求"]。

.開始之前
* 現有叢集和網路組態如下：
+
** Nexus 3132Q-V 叢集基礎架構是備援的，並且在兩台交換器上都能完全正常運作。
+
link:https://mysupport.netapp.com/site/products/all/details/cisco-cluster-storage-switch/downloads-tab["Cisco 乙太網路交換器"^] 擁有適用於交換器的最新 RCF 和 NX-OS 版本。

** 所有叢集連接埠均處於 `up`狀態。
** 兩台交換器都具備管理連線功能。
** 所有叢集邏輯介面（LIF）均處於 `up`狀態並已移轉。


* 對於 Nexus 3132Q-V 的替換交換器，請確保：
+
** 替換交換器的管理網路連線功能正常。
** 已具備對替換開關的控制台存取權限。
...
此程序將第二個 Nexus 3132Q-V 叢集交換器 CL2 替換為新的 3132Q-V 交換器 C2。
...
* 節點編號為 n1、n2、n3 和 n4。本流程中的範例使用四個節點：兩個節點使用四個 10 GB 叢集互連連接埠：e0a、e0b、e0c 和 e0d。另外兩個節點使用兩個 40 GB 叢集互連連接埠：e4a 和 e4e。請參閱 link:https://hwu.netapp.com/["Hardware Universe"^] 以了解您平台上的實際叢集連接埠。
...
* 叢集交換器 CL2 將由 C2 取代。
+
** 在每個節點上，連接到 CL2 的叢集 LIF 會移轉至連接到 CL1 的叢集連接埠。
** 中斷 CL2 上所有連接埠的纜線連接，然後將纜線重新連接到替換交換器 C2 上的相同連接埠。
** 在每個節點上，其已移轉的叢集 LIF 將會回復。
...
. 確認新的 Nexus 3132Q-V 交換器上已根據您的要求安裝了適當的 RCF 和映像，並進行任何必要的站台自訂。
+
此時您必須準備好替換交換器。如果您需要升級 RCF 和映像，則必須按照以下步驟操作：

+
.. 在 NetApp 支援網站上，請參閱 link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch/["Cisco乙太網路交換機"^]。
...
. 拆下連接到 Nexus 3132Q-V 交換器 CL2 的所有纜線，並將其重新連接到所有節點上的替換交換器 C2。
. 從 CL2 上的 e1/31 和 e1/32 連接埠移除 ISL 纜線，然後將其重新連接到替換交換器 C2 上的相同連接埠。
...
+
連接埠 Eth1/31 和 Eth1/32 應顯示 `(P)`，這表示 ISL 連接埠在連接埠通道中已啟動。
...
+
連接埠 Eth1/31 和 Eth1/32 應顯示 `(P)`，這表示兩個 ISL 連接埠都在連接埠通道中運作。
...
----
====
. 在所有節點上，啟動連接到 Nexus 3132Q-V 交換器 C2 的所有叢集互連埠：
+
`network port modify`
...
[%collapsible]
====
此範例顯示所有 LIF 均已成功回復，因為在  `Current Port` 欄下所列的連接埠在  `Is Home` 欄中的狀態為  `true`。如果  `Is Home` 欄值為  `false`，則 LIF 尚未回復。
...
----
====
. 如果更換後的 Nexus 3132Q-V 交換器尚未自動移除，請將其移除：
+
