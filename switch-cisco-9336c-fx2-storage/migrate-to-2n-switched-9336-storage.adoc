...
summary: 將現有的雙節點無交換器式叢集移轉至使用 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 儲存設備交換器的交換式叢集環境。 
---
= 使用 Cisco Nexus 9336C-FX2 儲存設備交換器移轉至雙節點交換式叢集
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 儲存交換器，將現有的雙節點無交換器式叢集移轉至交換式叢集環境。這種不中斷營運的移轉可讓您將叢集擴展至兩個節點以上。
...
* 看 link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe- 交換機"^] 有關佈線的更多資訊。
* 看 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^] 有關交換器安裝要求的詳細資訊。
...
. 對於ONTAP 9.8 及更高版本，請使用以下命令啟用乙太網路交換器運行狀況監視器日誌收集功能，以收集與交換器相關的日誌檔案：
+
...
. 對於ONTAP版本 9.5P16、9.6P12 和 9.7P10 及更高版本的修補程式，請使用以下命令啟用乙太網路交換器運行狀況監視器日誌收集功能，以收集與交換器相關的日誌檔案：
+
