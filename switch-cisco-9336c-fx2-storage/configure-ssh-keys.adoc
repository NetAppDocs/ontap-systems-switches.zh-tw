...
summary: 確認 Cisco Nexus 9336C-FX2 儲存設備交換器上已啟用 SSH 和 SSH 金鑰，以支援 CSHM 和日誌收集。 
---
= 驗證 Cisco Nexus 9336C-FX2 儲存設備交換器的 SSH 組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
確認 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 儲存設備交換器上已啟用 SSH 和 SSH 金鑰，以支援乙太網路交換器健全狀況監視器 （CSHM） 和日誌收集功能。
...
IMPORTANT: 啟用 FIPS 時，必須使用指令 `ssh key ecdsa 256 force`將交換器上的位元計數變更為 256。如需更多詳細資料，請參閱link:https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips["使用 FIPS 設定網路安全"^]。
