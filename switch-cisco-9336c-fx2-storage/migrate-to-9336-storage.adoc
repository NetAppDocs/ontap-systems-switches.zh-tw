...
summary: 將舊的 Cisco 交換器移轉至 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 儲存交換器，而不中斷儲存網路運作。 
---
= 將舊款 Cisco 交換器移轉至 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 儲存交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將舊的 Cisco 交換器移轉至 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 儲存交換器，而不中斷儲存網路運作。
...
.關於範例
本流程中的範例使用了兩個節點。這些節點使用兩個 10GbE 叢集互連連接埠 e0a 和 e0b。參見 https://hwu.netapp.com/["Hardware Universe"^]驗證平台上的叢集連接埠是否正確。看 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安裝HWU中沒有的設備還需要哪些額外資訊？"^] 有關交換器安裝要求的詳細資訊。
...
--
====
. [[步驟6]]如果您已停用自動建立案例功能，請透過呼叫AutoSupport訊息重新啟用此功能： `system node autosupport invoke -node * -type all -message MAINT=END`
