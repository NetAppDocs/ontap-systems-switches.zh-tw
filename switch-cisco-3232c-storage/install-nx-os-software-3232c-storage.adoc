...
.開始之前
請確認您已具備以下條件：* 交換器設定的最新備份。* 叢集運作正常（日誌中無錯誤或類似問題）。* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco乙太網路交換器頁面"^]。請查閱交換器相容性列表，以了解支援的 ONTAP 和 NX-OS 版本。* link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["CiscoNexus 3000 系列交換機"^]。請參閱 Cisco 網站上提供的相應軟體和升級指南，以取得 Cisco 交換器升級和降級步驟的完整文件。
