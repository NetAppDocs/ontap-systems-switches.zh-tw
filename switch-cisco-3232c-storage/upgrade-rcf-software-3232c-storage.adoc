...
summary: '當有新版本可用時，請升級 Cisco Nexus 3232C 儲存設備交換器上的參考組態檔案 (RCF)。' 
---
= 升級 Cisco Nexus 3232C 儲存設備交換器上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 Cisco Nexus 3232C 儲存設備交換器有更新版本的參考組態檔案 （RCF） 時，請升級 RCF。
...
CAUTION: 請務必關閉*所有*已連接的連接埠，以避免任何網路連線問題。如需更多詳細資訊，請參閱知識庫文章 https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["在切換作業系統升級期間遷移叢集 LIF 時，節點脫離仲裁。"^]。
...
+
請參閱知識庫文章 link:https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["如何在保持遠端連線的情況下清除Cisco互連交換器上的配置"^] 以了解更多詳情。
